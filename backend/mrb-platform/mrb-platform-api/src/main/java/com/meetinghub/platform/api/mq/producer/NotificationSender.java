 ...
 * </p>
 * <p>
 * {@link NotificationProducer} 通过 {@link org.springframework.beans.factory.ObjectProvider} 弱依赖注入：
 * 服务配置了 {@code rocketmq.name-server} 时走 MQ 优先链路；未配置时直接降级 Feign 同步调用。
 * </p>
 * <ul>
 *   <li>正常链路：{@link NotificationProducer#send} 同步发送至 mrb-notification topic</li>
 *   <li>MQ 异常或 Producer 缺失：降级 {@link NotificationFeignClient#sendBatch} 同步直发</li>
...
public class NotificationSender {
    ...
    private final ObjectProvider<NotificationProducer> notificationProducerProvider;
    ...
    public void sendSafe(List<Long> userIds, NotificationSendDTO template) {
        ...
        NotificationProducer producer = notificationProducerProvider.getIfAvailable();
        if (producer != null) {
            try {
                producer.send(userIds, template);
                return;
            } catch (Exception e) {
