...
public class FileServiceImpl implements FileService {
    ...
    public FileUploadVO upload(MultipartFile file, FileBizType bizType) {
        ...
        try {
            fileStorageService.store(file.getBytes(), objectKey, file.getContentType());
            // 上传成功后即时生成预签名 URL，供前端短期预览
            long expire = properties.getCos().getPresignedExpire();
            String url = fileStorageService.generatePresignedUrl(objectKey, expire);
