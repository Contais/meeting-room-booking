...
public class KnowledgeTool {
    ...
    public KnowledgeSearchResult searchKnowledge(
        ...
        try {
            ...
            if (result == null || result.getData() == null || result.getData().isEmpty()) {
                return KnowledgeSearchResult.miss();
            }
            List<KnowledgeHit> entries = result.getData().stream()
                    .map(dto -> new KnowledgeHit(dto.getTitle(), dto.getCategory(), dto.getAnswer(), dto.getTags()))
                    .toList();
            return new KnowledgeSearchResult(entries, false);
        } catch (Exception e) {
            log.error("searchKnowledge 调用失败, query={}", query, e);
            return new KnowledgeSearchResult(List.of(), true, "知识库检索暂时不可用，请稍后重试");
        }
    }
     ...
     */
    public record KnowledgeHit(String title, String category, String answer, String tags) {
    }
     ...
     */
    public record KnowledgeSearchResult(List<KnowledgeHit> entries, boolean empty, String hint) implements ToolResult {

        public KnowledgeSearchResult(List<KnowledgeHit> entries, boolean empty) {
            this(entries, empty, null);
        }

        public static KnowledgeSearchResult miss() {
