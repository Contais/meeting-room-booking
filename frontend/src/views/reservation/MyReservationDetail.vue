<template>
  <div class="page-view">
      ...
      <div class="detail-card-body">
        ...
        <div v-if="reservation" class="attendee-section">
          <div class="section-title-bar">
            ...
            <el-button v-if="canInvite" type="primary" plain size="small" @click="inviteDialogVisible = true">
    ...
    <!-- 邀请参会人弹窗（复用通用人员选择器，交互与预约表单一致） -->
    <UserSelectDialog
      v-model="inviteDialogVisible"
      title="邀请参会人"
      :locked-users="lockedAttendees"
...
import { ArrowLeft, Check, Close, Delete, Plus } from '@element-plus/icons-vue'
import {
...
import { respondInvitation, inviteAttendees } from '@/api/attendee'
...
/** 锁定不可选人员：预约创建者 + 已邀请参会人（并展示查阅状态） */
const lockedAttendees = computed(() => {
  const list: Array<{ id: string; label: string; type: 'info' | 'success' | 'danger' | 'warning' }> = []
  if (reservation.value?.userId) {
    list.push({ id: reservation.value.userId, label: '创建者', type: 'warning' })
  }
  for (const a of reservation.value?.attendees || []) {
    if (a.userId === reservation.value?.userId) continue
    list.push({
      id: a.userId,
      label: attendeeStatusText(a.status),
      type: attendeeStatusType(a.status),
    })
  }
  return list
})

async function onInviteConfirm(ids: string[]) {
  ...
  try {
    const res = await inviteAttendees(reservation.value.id, ids)
