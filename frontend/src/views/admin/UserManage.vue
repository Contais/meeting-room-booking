...
const filterDefs = [
  { prop: 'keyword', label: '搜索', type: 'input' as const, placeholder: '用户名/手机号' },
...
async function loadData() {
  loading.value = true
  try { const res = await listUsers(query); tableData.value = res.data.records; total.value = res.data.total } catch { /* */ } finally { loading.value = false }
}
...
async function handleSubmit() {
  ...
  try { if (isEdit.value) { await updateUser(form); ElMessage.success('更新成功') } else { await createUser(form); ElMessage.success('创建成功') }; dialogVisible.value = false; loadData() } catch { /* */ } finally { submitting.value = false }
}
async function handleToggleStatus(row: UserInfo) { try { await toggleUserStatus(row.id); ElMessage.success('操作成功'); loadData() } catch { /* */ } }
async function handleDelete(id: number) { try { await deleteUser(id); ElMessage.success('删除成功'); loadData() } catch { /* */ } }
onMounted(loadData)
</script>
