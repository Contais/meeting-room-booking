  ...
  <div class="room-manage">
    ...
    <FilterBar :model="query" @search="loadData" @reset="resetQuery">
      <el-form-item label="搜索">
        <el-input v-model="query.keyword" placeholder="名称/位置" clearable style="width: 200px" @keyup.enter="loadData" />
      </el-form-item>
    ...
    <div class="table-card page-card">
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column label="可预约时段" width="130"><template #default="{ row }">{{ row.bookableStart || '08:00' }} ~ {{ row.bookableEnd || '20:00' }}</template></el-table-column>
        <el-table-column label="审批" width="70"><template #default="{ row }"><el-tag :type="row.needApproval === 1 ? 'warning' : 'success'" size="small" effect="dark" round>{{ row.needApproval === 1 ? '需审批' : '免审批' }}</el-tag></template></el-table-column>
        <el-table-column prop="status" label="状态" width="70"><template #default="{ row }"><el-tag :type="row.status === 1 ? 'success' : 'info'" effect="dark" round size="small">{{ row.status === 1 ? '启用' : '禁用' }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑会议室' : '新增会议室'" width="640px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="设备设施"><el-input v-model="form.equipment" placeholder="如：投影仪,白板,视频会议系统" /></el-form-item>
        <el-form-item label="描述"><el-input v-model="form.description" type="textarea" :rows="2" placeholder="会议室详细描述" /></el-form-item>
        <el-divider content-position="left">使用规则</el-divider>
        <el-form-item label="可预约时段"><div style="display: flex; align-items: center; gap: 8px"><el-time-picker v-model="form.bookableStart" format="HH:mm" value-format="HH:mm" placeholder="开始时间" style="width: 140px" /><span>~</span><el-time-picker v-model="form.bookableEnd" format="HH:mm" value-format="HH:mm" placeholder="结束时间" style="width: 140px" /></div></el-form-item>
        <el-form-item label="最大预约时长"><el-input-number v-model="form.maxDuration" :min="30" :max="1440" :step="30" style="width: 200px" /><span style="margin-left: 8px; color: #9ca3af; font-size: 13px">分钟</span></el-form-item>
        <el-form-item label="提前预约天数"><el-input-number v-model="form.advanceDays" :min="1" :max="90" style="width: 200px" /><span style="margin-left: 8px; color: #9ca3af; font-size: 13px">天</span></el-form-item>
        <el-form-item label="审批模式"><el-radio-group v-model="form.needApproval"><el-radio :value="0">免审批</el-radio><el-radio :value="1">需管理员审批</el-radio></el-radio-group></el-form-item>
      </el-form>
...
const rules: FormRules = { name: [{ required: true, message: '请输入会议室名称', trigger: 'blur' }], capacity: [{ required: true, message: '请输入容纳人数', trigger: 'blur' }] }
