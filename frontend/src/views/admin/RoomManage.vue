    ...
    <FilterTable :filters="filterDefs" :filter-model="query" @search="loadData" @reset="resetQuery">
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column label="可预约时段" width="130">
          <template #default="{ row }">{{ row.bookableStart || '08:00' }} ~ {{ row.bookableEnd || '20:00' }}</template>
        </el-table-column>
        <el-table-column label="审批" width="70">
          <template #default="{ row }">
            <el-tag :type="row.needApproval === 1 ? 'warning' : 'success'" size="small" effect="dark" round>{{ row.needApproval === 1 ? '需审批' : '免审批' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" width="70">
          <template #default="{ row }">
            <el-tag :type="row.status === 1 ? 'success' : 'info'" effect="dark" round size="small">{{ row.status === 1 ? '启用' : '禁用' }}</el-tag>
          </template>
        </el-table-column>
...
const query = reactive<Record<string, any>>({ page: 1, size: 10, keyword: '', status: undefined })

const filterDefs = [
  { prop: 'keyword', label: '搜索', type: 'input' as const, placeholder: '名称/位置' },
  { prop: 'status', label: '状态', type: 'select' as const, options: [{ label: '启用', value: 1 }, { label: '禁用', value: 0 }] },
]
...
function resetQuery() { query.keyword = ''; query.status = undefined; query.page = 1 }
