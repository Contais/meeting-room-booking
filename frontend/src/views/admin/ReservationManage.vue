    ...
    <div class="search-bar">
      <div class="search-fields">
        <template v-if="!expanded">
          <div class="search-item"><el-input class="search-keyword-input" v-model="query.keyword" placeholder="搜索会议主题或联系电话" clearable @input="onSearchInput" /></div>
        </template>
        <template v-else>
          <div class="search-item"><label>会议主题</label><el-input v-model="query.subject" placeholder="搜索会议主题或联系电话" clearable @input="onSearchInput" /></div>
          <div class="search-item"><label>联系电话</label><el-input v-model="query.contactPhone" placeholder="请输入联系电话" clearable @input="onSearchInput" /></div>
          <div class="search-item"><label>状态</label><el-select v-model="query.status" placeholder="请选择" clearable @change="query.page = 1; loadData()"><el-option label="待确认" :value="0" /><el-option label="已确认" :value="1" /><el-option label="已取消" :value="2" /></el-select></div>
...
const query = reactive({ page: 1, size: 10, keyword: '', subject: '', contactPhone: '', startTime: '', status: undefined as number | undefined })
...
function resetQuery() { query.keyword = ''; query.subject = ''; query.contactPhone = ''; query.startTime = ''; query.status = undefined; query.page = 1; loadData() }
function onSizeChange() { query.page = 1; loadData() }
async function loadData() { loading.value = true; try { const res = await listAllReservations(query); tableData.value = res.data.records; total.value = Number(res.data.total) || 0 } catch { /* */ } finally { loading.value = false } }
