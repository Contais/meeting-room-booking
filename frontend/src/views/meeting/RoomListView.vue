<template>
  <div class="room-list">
    ...
    <div class="filter-bar page-card">
      <el-input v-model="filter.keyword" placeholder="搜索会议室名称..." clearable prefix-icon="Search" style="width: 260px" @input="handleFilter" />
      <el-input-number v-model="filter.minCapacity" :min="1" :max="1000" placeholder="最少人数" controls-position="right" style="width: 140px" @change="handleFilter" />
    ...
    <div class="room-grid">
      <div v-for="room in filteredRooms" :key="room.id" class="room-card" @click="goDetail(room.id)">
        ...
        <div class="room-actions">
          <el-button type="primary" size="small" round class="btn-gradient" @click.stop="goDetail(room.id)">
            查看详情
          </el-button>
        </div>
      </div>
...
const filteredRooms = computed(() => {
  return rooms.value.filter(room => {
    if (filter.keyword && !room.name.includes(filter.keyword) && !(room.location || '').includes(filter.keyword)) {
      return false
    }
    if (filter.minCapacity && (!room.capacity || room.capacity < filter.minCapacity)) {
      return false
    }
    return true
  })
