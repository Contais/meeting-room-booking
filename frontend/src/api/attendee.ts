...
export function inviteAttendees(reservationId: string, userIds: string[]): Promise<Result<number>> {
  return request.post(`/api/meeting/reservation/attendee/${reservationId}/invite`, { userIds })
}

/** 按部门邀请参会人，返回实际新增数量 */
export function inviteDepartment(reservationId: string, departmentId: string): Promise<Result<number>> {
  return request.post(`/api/meeting/reservation/attendee/${reservationId}/invite-department`, { departmentId })
}

/** 查询预约的参会人列表 */
export function listAttendees(reservationId: string): Promise<Result<Attendee[]>> {
  return request.get(`/api/meeting/reservation/attendee/${reservationId}/list`)
}

/** 移除参会人 */
export function removeAttendee(reservationId: string, userId: string): Promise<Result<void>> {
  return request.delete(`/api/meeting/reservation/attendee/${reservationId}/${userId}`)
}

/** 参会人响应邀请（更新自己的参会状态: 1-已接受, 2-已拒绝） */
export function respondInvitation(reservationId: string, status: number): Promise<Result<void>> {
