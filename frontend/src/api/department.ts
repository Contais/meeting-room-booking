...
export function createDepartment(data: { name: string; parentId?: string; sortOrder?: number }): Promise<Result<void>> {
  return request.post('/api/uc/department/admin/create', data)
}

export function updateDepartment(data: { id: string; name: string; parentId?: string; sortOrder?: number }): Promise<Result<void>> {
